...
@Component({
  selector: 'app-business-space',
  imports:[NavbarComponent,CommonModule,ReactiveFormsModule,SideBarComponent],
  templateUrl: './business-space.component.html',
  styleUrl: './business-space.component.css'
})
export class BusinessSpaceComponent implements OnInit  {
isLoading = false;
...
bookingHours: [''],
  ...
  // Step 6
allowVenueBookingRequests: [true],
...
getMissingFields(): string[] {
  ...
  if (!form.venueCategory)
    fields.push('Venue Category');
...
buildPayload(isDraft: boolean) {
  ...
  return {
    ...
    businessName:
      this.profileForm.value.businessName,
    ...
    category:
      this.profileForm.value.venueCategory,

    venueType:
      this.profileForm.value.venueType,
    ...
    capacityRange:
      this.profileForm.value.capacityRange,

    availabilityHours:
      this.profileForm.value.bookingHours,

    amenities:
      this.profileForm.value.amenities
        ? this.profileForm.value.amenities
            .split(',')
            .map((x: string) => x.trim())
        : [],
    ...
    allowDirectBooking:
      this.profileForm.value.allowVenueBookingRequests,
